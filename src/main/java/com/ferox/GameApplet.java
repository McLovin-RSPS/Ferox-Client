...
public class GameApplet extends Canvas implements Runnable, MouseListener, MouseMotionListener, MouseWheelListener,
    ...
    public void refreshFrameSize(boolean undecorated, int width, int height, boolean resizable,
                                 boolean full) {
        myWidth = width;
        myHeight = height;
    ...
    final void createClientFrame(int w, int h) {
        ...
        gameFrame = new GameFrame(this, myWidth, myHeight, Client.screen == ScreenMode.RESIZABLE, Client.screen == ScreenMode.FULLSCREEN);
    ...
    public void run() {
        ...
        do {
            ...
            if (shiftTeleport) {
                int newHeight = Client.instance.plane - mouseRotation;
                if (newHeight < 0) {
                    newHeight = 0;
                }
                if (newHeight > 3) {
                ...
                Client.teleport(Client.local_player.waypoint_x[0] + Client.instance.next_region_start, Client.local_player.waypoint_y[0] + Client.instance.next_region_end, newHeight);
                shiftTeleport = false;
            }
            runPendingTasks();
        } while (true);
    ...
    public void mouseWheelMoved(MouseWheelEvent event) {
        ...
        if (cursor_x > 0 && cursor_x < 512 && cursor_y > Client.window_height - 165 && cursor_y < Client.window_height - 25) {
            ...
            int zoom_in = HdPlugin.process() ? -100 : 100;

            int zoom_out = HdPlugin.process() ? 4800 : 1400;
            /** ZOOMING **/
            ...
            if (zoom && Client.widget_overlay_id == -1 && Client.instance.settings[ConfigUtility.ZOOM_TOGGLE_ID] == 0) {
                ...
                if (Client.zoom_distance > zoom_out) {
                    Client.zoom_distance = zoom_out;
                }
                if (Client.zoom_distance < zoom_in) {
                    Client.zoom_distance = zoom_in;
                }
    ...
    public void handleInterfaceScrolling(MouseWheelEvent event) {
        ...
        if (tabInterfaceId != -1) {
            handleScrolling(rotation, tabInterfaceId, Client.instance.screen == ScreenMode.FIXED ? Client.instance.getScreenWidth() - 218
                : (Client.instance.screen == ScreenMode.FIXED ? 28
                : Client.instance.getScreenWidth() - 197), Client.instance.screen == ScreenMode.FIXED ? Client.instance.getScreenHeight() - 298
                : (Client.instance.screen == ScreenMode.FIXED ? 37
                : Client.instance.getScreenHeight()
                - (Client.instance.getScreenWidth() >= 1000 ? 37 : 74) - 267));
        }
        if (Client.instance.widget_overlay_id != -1) {
            handleScrolling(rotation, Client.instance.widget_overlay_id, Client.instance.screen == ScreenMode.FIXED ? 4
                : (Client.instance.getScreenWidth() / 2) - 356, Client.instance.screen == ScreenMode.FIXED ? 4
                : (Client.instance.getScreenHeight() / 2) - 230);
        }
    }
    ...
    public final void mouseDragged(MouseEvent e) {
        ...
        if (mouse_wheel_down && Client.instance.settings[ConfigUtility.MOUSE_CAMERA_ID] == 1) {
