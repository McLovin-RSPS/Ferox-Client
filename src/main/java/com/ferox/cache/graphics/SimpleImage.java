...
public final class SimpleImage extends Rasterizer2D {
    ...
    public void drawAdvancedTransparentSprite(int x, int y, int opacity) {
        ...
        if (y + k1 > Rasterizer2D.clip_bottom) {
            k1 -= (y + k1) - Rasterizer2D.clip_bottom;
        }
        ...
        if (x + l1 > Rasterizer2D.clip_right) {
            int i3 = (x + l1) - Rasterizer2D.clip_right;
            l1 -= i3;
            j2 += i3;
            i2 += i3;
        }
        if (!(l1 <= 0 || k1 <= 0)) {
            method351(j1, l1, Rasterizer2D.pixels, pixels, j2, k1, i2, k, i1);
        }
    }
        ...
        for (int column = -height; column < 0; column++) {
            for (int row = -width; row < 0; row++) {
                ...
                if (color != 0) {
                    ...
                    drawAlpha(pixels, dst_pos++, ((color & 0xff00ff) * alpha + (src & 0xff00ff) * opacity & 0xff00ff00) + ((color & 0xff00) * alpha + (src & 0xff00) * opacity & 0xff0000) >> 8, color, alpha);
                } else {
                    dst_pos++;
                }
            }
        ...
        try {
            ...
            for (y = 0; y < height; y++) {
                ...
                for (x = -dst_width[y]; x < 0; x++) {
                    ...
                    Rasterizer2D.drawAlpha(Rasterizer2D.pixels, index++, red | green | blue, 255);
                    a += cos;
                    b -= sin;
                }
                rot_x += sin;
    ...
    private void renderARGBPixels(int spriteWidth, int spriteHeight, int spritePixels[], int renderAreaPixels[], int pixel, int alphaValue, int i, int l, int j1) {
        ...
        for (int height = -spriteHeight; height < 0; height++) {
            for (int width = -spriteWidth; width < 0; width++) {
                ...
                if (pixelColor != 0) {
                    int pixelValue = renderAreaPixels[pixel];
                    drawAlpha(renderAreaPixels, pixel++, ((pixelColor & 0xff00ff) * alphaValue + (pixelValue & 0xff00ff) * alphaLevel & 0xff00ff00) + ((pixelColor & 0xff00) * alphaValue + (pixelValue & 0xff00) * alphaLevel & 0xff0000) >> 8, pixelColor, alpha);
                } else {
                    pixel++;
                }
            }
