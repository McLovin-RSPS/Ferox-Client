                    ...
                    if (effect.equals(lt_string)) {
                        ...
                        if (effect.startsWith(set_image)) {
                            try {
                                ...
                                int height_offset = icon.height - 2;//11;//changed to max_height
                                if (opacity == 256) {
        ...
        for (int column = -height; column < 0; column++) {
            for (int row = length; row < 0; row++) {
                if (glyph_pixels[src_pos++] != 0) {
                    drawAlpha(pixels, dst_pos++, color, 255);
                } else {
                    dst_pos++;
                }
                if (glyph_pixels[src_pos++] != 0) {
        ...
        for (int column = -height; column < 0; column++) {
            for (int row = -width; row < 0; row++) {
                if (glyph_pixels[src_pos++] != 0) {
                    ...
                    drawAlpha(pixels, dst_pos++, ((((src & 0xff00ff) * alpha & ~0xff00ff) + ((src & 0xff00) * alpha & 0xff0000)) >> 8) + color, 255);
                } else {
                    dst_pos++;
                }
            }
