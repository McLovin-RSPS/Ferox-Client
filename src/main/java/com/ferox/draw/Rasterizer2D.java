...
public class Rasterizer2D extends Cacheable {
     ...
     */
    public static void drawBox(int leftX, int topY, int width, int height, int rgbColour, boolean itemSprite) {
        try {
            if (leftX < Rasterizer2D.clip_left) {
                width -= Rasterizer2D.clip_left - leftX;
                leftX = Rasterizer2D.clip_left;
            ...
            for (int rowIndex = 0; rowIndex < height; rowIndex++) {
                for (int columnIndex = 0; columnIndex < width; columnIndex++) {
                    drawAlpha(pixels, pixelIndex++, rgbColour, itemSprite ? 0 : 255);
                }
                pixelIndex += leftOver;
            }
        }catch (Exception e) {
    ...
    public static void draw_filled_rect(int leftX, int topY, int width, int height, int rgbColour, int opacity) {
        ...
        for (int rowIndex = 0; rowIndex < height; rowIndex++) {
            for (int columnIndex = 0; columnIndex < width; columnIndex++) {
                ...
                drawAlpha(pixels, pixelIndex++, transparentColour, 255);
            }
            pixelIndex += leftOver;
        }
    }
    ...
    public static void draw_vertical_line(int x, int y, int length, int color, int alpha) {
            ...
            if (alpha == 256) {
                ...
                while (i_26_ < y) {
                    ...
                    for (int i_35_ = i_32_; i_35_ < i_33_; i_35_++) {
                        ...
                        drawAlpha(pixels, coordinates++, dest_color, 255);
                    }
                    i_26_++;
                    i_30_ -= i_29_-- + i_29_;
                    i_31_ -= i_29_ + i_29_;
                ...
                while (i_26_ < diameter_end) {
                    ...
                    for (int i_43_ = i_40_; i_43_ <= i_41_; i_43_++) {
                        ...
                        drawAlpha(pixels, coordinates++, i_47_, 255);
                    }
                    i_26_++;
                    i_31_ += i_29_ + i_29_;
                    i_30_ += i_29_++ + i_29_;
    ...
    private static final void setPixel(final int x, final int y, final int color) {
        if (x >= clip_left && y >= clip_top && x < clip_right && y < clip_bottom) {
            drawAlpha(pixels, x + y * width, color, 255);
        }
    }
        ...
        if (radius == 0) {
            ...
            while (i_69_ < y) {
                ...
                for (int i_78_ = i_75_; i_78_ < i_76_; i_78_++) {
                    drawAlpha(pixels, i_77_++, color, 255);
                }
                i_69_++;
                i_73_ -= i_72_-- + i_72_;
                i_74_ -= i_72_ + i_72_;
            ...
            if (line_width > line_height) {
                ...
                while (y <= line_height) {
                    ...
                    if (width_step >= clip_left && width_step < clip_right) {
                        drawAlpha(pixels, width_step + width * y, color, 255);
                    }
                    x += height_step;
                    y++;
                }
