...
public final class Rasterizer3D extends Rasterizer2D {
    ...
    public static void drawShadedTriangle(int y_a, int y_b, int y_c, int x_a, int x_b, int x_c, int hsl1, int hsl2, int hsl3, float z_a, float z_b, float z_c) {
        if(HdPlugin.process() && !renderOnGpu)
            return;
    ...
    public static void drawShadedScanline(int[] dest, int offset, int x1, int x2, int r1, int g1, int b1, int r2, int g2, int b2, float depth, float depth_slope) {
        ...
        if (x1 < x2) {
            ...
            if (alpha == 0) {
                while (--n >= 0) {
                    if (true) {
                        drawAlpha(dest, offset, (r1 & 0xff0000) | (g1 >> 8 & 0xff00) | (b1 >> 16 & 0xff), 255);
                ...
                while (--n >= 0) {
                    ...
                    dst = dest[offset];
                    if(true) {
                        drawAlpha(dest, offset, rgb + ((dst & 0xff00ff) * a1 >> 8 & 0xff00ff) + ((dst & 0xff00) * a1 >> 8 & 0xff00), 255);
                        Rasterizer2D.depth_buffer[offset] = depth;
                    }
                    depth += depth_slope;
                    r1 += r2;
    ...
    public static int media_length = 94;
    ...
    public static void drawShadedTriangle(int y1, int y2, int y3, int x1, int x2, int x3, int hsl1, int hsl2, int hsl3) {
        if(HdPlugin.process() && !renderOnGpu)
            return;
        if (true) {
            drawShadedTriangle(y1, y2, y3, x1, x2, x3, hsl1, hsl2, hsl3, 0, 0, 0);
            return;
        }
        int j2 = 0;
        int k2 = 0;
        ...
        if (low_detail) {
            ...
            if (opaque) {
                ...
                while (n-- > 0) {
                    ...
                    l = l1 >> 16;
                    drawAlpha(raster, k++, ((rgb & 0xff00ff) * l & ~0xff00ff) + ((rgb & 0xff00) * l & 0xff0000) >> 8 , rgb, 255);
                    uA += i7;
                    vA += k7;
                    l1 += dl;
                    rgb = texels[(vA & 0xfc0) + (uA >> 6) & (repeatTexture ? 4095 : (vA & 0xfc0) + (uA >> 6))];
            ...
            while (n-- > 0) {
                ...
                if ((k8 = texels[(vA & 0xfc0) + (uA >> 6) & (repeatTexture ? 4095 : (vA & 0xfc0) + (uA >> 6))]) != 0) {
                    l = l1 >> 16;
                    drawAlpha(raster, k, ((k8 & 0xff00ff) * l & ~0xff00ff) + ((k8 & 0xff00) * l & 0xff0000) >> 8 , k8, 255);
                }
                k++;
                uA += i7;
                vA += k7;
            ...
            for (n = x2 - x1 & 7; n-- > 0; ) {
                ...
                if ((l8 = texels[(vA & 0xfc0) + (uA >> 6) & (repeatTexture ? 4095 : (vA & 0xfc0) + (uA >> 6))]) != 0) {
                    l = l1 >> 16;
                    drawAlpha(raster, k, ((l8 & 0xff00ff) * l & ~0xff00ff) + ((l8 & 0xff00) * l & 0xff0000) >> 8 , l8, 255);
                }
                k++;
                uA += i7;
                vA += k7;
    ...
    private static void drawFlatScanline(int dest[], int offset, int rgb, int x1, int x2) {
        ...
        while (--pos >= 0) {
            for (int i = 0; i < 4; i++) {
                drawAlpha(dest, offset, rgb + ((dest[offset] & 0xff00ff) * a1 >> 8 & 0xff00ff) + ((dest[offset] & 0xff00) * a1 >> 8 & 0xff00), rgb, 255);
                offset++;
            }
        }
        for (pos = x2 - x1 & 3; --pos >= 0; ) {
