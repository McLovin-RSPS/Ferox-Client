...
public class HoverMenuManager {
    ...
    public static void init() {
        ...
        menus.put(HOLY_SCYTHE_OF_VITUR, new HoverMenu("@red@+10%@whi@ accuracy bonus"));
        menus.put(HOLY_GHRAZI_RAPIER, new HoverMenu("A much more powerful version of the Ghrazi rapier"));
        menus.put(SANGUINE_SCYTHE_OF_VITUR, new HoverMenu("A much stronger version then the Scythe of vitur"));
        menus.put(SANGUINE_TWISTED_BOW, new HoverMenu("A much stronger version then the Twisted bow"));
        menus.put(TWISTED_BOW_I, new HoverMenu("An untradeable, recoloured version of the Twisted bow"));
        ...
        menus.put(RING_OF_TRINITY, new HoverMenu("The effects of all 3 forged rings combined into one"));
        ...
        menus.put(KEY_OF_DROPS, new HoverMenu("When using the key on a mystery box you get 2 rolls on the table instead of 1"));
    ...
    public static int drawType() {
        if (Client.instance.cursor_x > 0 && Client.instance.cursor_x < 500 && Client.instance.cursor_y > 0
            && Client.instance.cursor_y < 300) {
            return 1;
        }
        return 0;
    }
    ...
    public static void drawHintMenu() {
        int cursor_x = Client.instance.cursor_x;
        int cursor_y = Client.instance.cursor_y;
        if (!canDraw()) {
            return;
        }

        if (Client.screen != Client.ScreenMode.FIXED) {
            if (Client.instance.cursor_y < Client.window_height - 450
                && Client.instance.cursor_x < Client.window_width - 200) {
                return;
            }
            cursor_x -= 100;
            cursor_y -= 50;
        ...
        if (Client.screen == Client.ScreenMode.FIXED) {
            if (Client.instance.cursor_y < 210 || Client.instance.cursor_x < 561) {
            } else {
                cursor_x -= 516;
                cursor_y -= 158;
            }
            if (Client.instance.cursor_x > 600 && Client.instance.cursor_x < 685) {
                cursor_x -= 60;

            }
            if (Client.instance.cursor_x > 685) {
                cursor_x -= 80;
            }
        }
