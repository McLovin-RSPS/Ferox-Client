...
public final class Region {
    ...
    public final byte[][][] overlay_floor_id;
    ...
    public final byte[][][] underlay_floor_id;
    ...
    private void render(int y, SceneGraph scene, CollisionMap map, int type, int plane, int x, int object_id, int orientation) {
        ...
        if (low_detail && (tile_flags[0][x][y] & BRIDGE_TILE) == 0) {
            ...
            if (get_visibility(y, plane, x) != Region.plane)
                return;
        }
        //}
        if (plane < min_plane)
        ...
        if (orientation == 1 || orientation == 3) {
            size1 = def.height;//objectSizeY
            size2 = def.width;//objectSizeX
        } else {
            size1 = def.width;
            size2 = def.height;
        }
        ...
        int v_sw = vertex_heights[plane][modX][modY];
        int v_se = vertex_heights[plane][modX1][modY];
        int v_ne = vertex_heights[plane][modX1][modY1];
        int v_nw = vertex_heights[plane][modX][modY1];
